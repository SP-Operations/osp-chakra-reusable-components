...
export function RIPlanItem({ plan, onChange }: RIPlanItemProps) {
    ...
    return (
        ...
        <CheckboxCard.Root my={2} variant={"subtle"} colorPalette={"green"} cursor={"pointer"} onClick={onOpen} checked={isChecked}>
            ...
            <CheckboxCard.Control>
                ...
                <BaseButton size="sm" variant={"ghost"} onClick={(e) => {e.stopPropagation(); onOpen();}}>
                    <LuInfo />
                </BaseButton>
            </CheckboxCard.Control>
        </CheckboxCard.Root>
                        ...
                        <Dialog.Body>
                            <Stack direction={{ base: "column", md: "row" }}>
                                ...
                                <SimpleGrid columns={{ base: 1, md: 2 }} px={1} width={"full"}>
                                    <Box px={3} py={3}>
                                        ...
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="LPA Number" value={plan.lpaNo} name="lpaN" readOnly />
                                            <InputFloatingLabel label="Account Status" value={plan.status} name="status" readOnly />
                                        </Flex>
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="Total Amount Payable" value={plan.totalAmtPayable} name="tap" readOnly />
                                            <InputFloatingLabel label="Total Amount Paid" value={plan.totalAmtPaid} name="totAmtPd" readOnly />
                                        </Flex>
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="Balance" value={plan.balance} name="bal" readOnly />
                                            <InputFloatingLabel label="Installment Amount" value={plan.instAmt} name="instAmt" readOnly />
                                        </Flex>

                                        <Checkbox.Root checked={isFullyPaid} colorPalette={"green"} display={{base: "none", md: "block"}} disabled={isChecked}>
                                            <Checkbox.HiddenInput onChange={() => setIsFullyPaid(checked => !checked)}/>
                                            <Checkbox.Control/>
                                    ...
                                    <Box p={3} bg={"gray.50"} border={"1px solid #ddd"} borderRadius="md">
                                        ...
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="LPA Number" value={plan.newLpaNo} name="lpaN" readOnly />
                                            <InputFloatingLabel label="Account Status" value={plan.newStatus} name="status" readOnly />
                                        </Flex>
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="Total Amount Payable" value={plan.newTotalAmtPayable} name="tap" readOnly />
                                            <InputFloatingLabel label="Total Amount Paid" value={plan.newTotalAmtPaid} name="totAmtPd" readOnly />
                                        </Flex>
                                        <Flex justifyContent={"space-between"} alignItems={"center"} my={3} gap={2}>
                                            <InputFloatingLabel label="Balance" value={plan.newBalance} name="bal" readOnly />
                                            <InputFloatingLabel label="Installment Amount" value={plan.newInstAmt} name="instAmt" readOnly />
                                        </Flex>
                                    </Box>
                                </SimpleGrid>
                            </Stack>

                            <Checkbox.Root checked={isFullyPaid} colorPalette={"green"} display={{base: "block", md: "none"}} disabled={isChecked} m={4}>
                                <Checkbox.HiddenInput onChange={() => setIsFullyPaid(checked => !checked)}/>
                                <Checkbox.Control/>
                                <Checkbox.Label> Reinstate Fully Paid </Checkbox.Label>
                            </Checkbox.Root>
                            ...
                            >
                                ...
                                <VStack align="start" p={1} mt={2}>
                                    <Text>
                                    Reinstatement Fee: <strong>₱ {reinstatementFee.toLocaleString()}</strong>
                                    </Text>
                                    <Text>
                            ...
                            <Box mt={3} mx={"auto"} textAlign={"center"}>
                                <Button 
                                bg={isChecked ? "red.600" : "var(--chakra-colors-primary)"} 
                                onClick={
                                    () => {
